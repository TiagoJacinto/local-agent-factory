...
main(async ({ prompt, config, adwId }) => {
  ...
  await run.phase({ name: "request", kind: "engineer", owner: run.engineer,
    description: "Capture the incoming ask" }, ph => ph.log({ input: prompt }));
  let plan: any;
  await run.phase({ name: "plan", kind: "agent", owner: "planner",
    description: "Turn the request into an implementable plan" },
    async ph => { plan = await ph.call(new AgentCall("PlanOutput", prompt, undefined,
      [gates.artifactsExist, gates.filesNonEmpty])); });
  await run.phase({ name: "build", kind: "agent", owner: "builder", retries: 1,
