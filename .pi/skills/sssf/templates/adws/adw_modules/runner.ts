...
export class Run {
	...
	writeEvidence(name: string, value: unknown) {
		...
		return atomicWrite(
			resolve(this.runEvidenceDir, name),
	...
	private sourceIntegrityError(state = this.finalSourceState()) {
		if (!state)
			return "source integrity check failed: source state is unavailable";
	...
	private finalize(ok: boolean, reason = "", statusOverride?: string) {
		...
		const finalSource = this.sourceRevision
			? this.finalSourceState()
			: undefined;
		const integrityError = this.sourceRevision
			? this.sourceIntegrityError(finalSource)
			: undefined;
	...
	async phase(
		...
		if (this.signal.aborted)
			throw new Error(this.abortReason || "workflow canceled");
		if (
