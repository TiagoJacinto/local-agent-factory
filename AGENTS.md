...
- `src/` and `src/skills/` are canonical source. `dist/` is generated package output. `laf init` writes only target-repository configuration; runtime evidence belongs in the database path selected by that configuration.
