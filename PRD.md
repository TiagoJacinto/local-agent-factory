...
**Product status:** Phase 0 is implementation-ready; Phases 1–13 are roadmap only
...
The design direction must not make robustness infrastructure a prerequisite for product value. Databases, retries, durable execution, tracing, sandbox platforms, and provider routing could consume substantial implementation time before the engineer has one usable end-to-end workflow.

The product will grow through narrow, user-visible workflow increments. Each delivery phase must be independently specified, implemented, and accepted before the next phase becomes implementation scope.

## Product Principles

- Define workflows as ordinary typed code rather than YAML, a visual workflow builder, or prompt-defined outer control flow.
- Keep workflow control flow outside model prompts.
- Treat the Workflow Executor as the primary product and testing seam, with adapters at external boundaries.
- Run generated changes in isolated local workspaces and preserve human authority over integration.
- Prefer the smallest valuable end-to-end workflow increment over speculative platform infrastructure.
- Keep optional technical improvements in `ROBUSTNESS.md`. They become implementation scope only when the owner explicitly promotes them into a phase specification.
- A roadmap entry is not agent-ready. Before implementation, its phase must define detailed user stories, acceptance behavior, testing decisions, dependencies, and exclusions.

## Delivery Roadmap

Each phase adds one visible workflow capability. Only phases marked **implementation-ready** may be broken into implementation tickets. Later phases remain directional briefs until the owner promotes and specifies them.

The linked robustness ideas are recommendations to reconsider during that phase, not automatic scope or completion gates.

| Phase | Specification status | Functional increment | Most relevant optional robustness ideas |
| --- | --- | --- | --- |
| **0 — Code-defined workflow kernel** | **Implementation-ready — current** | Execute Agent, Command, and Human Gate steps in order and return a reviewable local result. | [Current minimum policy](ROBUSTNESS.md#current-minimum-policy), [Capability ports](ROBUSTNESS.md#capability-ports), [Contract tests](ROBUSTNESS.md#contract-tests), and [Local run evidence](ROBUSTNESS.md#local-run-evidence). |
| **1 — Typed Change Plan** | Roadmap only | Add a planning step that produces objective, non-goals, expected files, risks, acceptance criteria, and validation commands. | [Untrusted-input boundaries](ROBUSTNESS.md#untrusted-input-boundaries) and [Local run evidence](ROBUSTNESS.md#local-run-evidence). |
| **2 — Configurable validation gates** | Roadmap only | Add, remove, and reorder formatter, lint, type-check, test, and repository-command steps. | [Timeout, cancellation, and process cleanup](ROBUSTNESS.md#timeout-cancellation-and-process-cleanup), [Contract tests](ROBUSTNESS.md#contract-tests), and [Local run evidence](ROBUSTNESS.md#local-run-evidence). |
| **3 — Independent review** | Roadmap only | Give a reviewer the plan, diff, and validation evidence before the Human Gate. | [Untrusted-input boundaries](ROBUSTNESS.md#untrusted-input-boundaries), [Changed-path policy](ROBUSTNESS.md#changed-path-policy), and [Local run evidence](ROBUSTNESS.md#local-run-evidence). |
| **4 — Review-to-build feedback** | Roadmap only | Return review findings to the builder through an explicit workflow edge. | [Bounded correction attempts](ROBUSTNESS.md#bounded-correction-attempts), [Retry classification](ROBUSTNESS.md#retry-classification), and [Run-state persistence and resume](ROBUSTNESS.md#run-state-persistence-and-resume). |
| **5 — Role-specific context packs** | Roadmap only | Give each step only the instructions, active requirements, and artifacts needed for its role. | [Untrusted-input boundaries](ROBUSTNESS.md#untrusted-input-boundaries), [Retrieval safety](ROBUSTNESS.md#retrieval-safety), and [Lesson provenance](ROBUSTNESS.md#lesson-provenance). |
| **6 — Task graph and bounded decomposition** | Roadmap only | Split a plan into scoped tasks with dependencies, owners, acceptance criteria, allowed files, and validation plans. | [Timeout, cancellation, and process cleanup](ROBUSTNESS.md#timeout-cancellation-and-process-cleanup), [Changed-path policy](ROBUSTNESS.md#changed-path-policy), and [Version compatibility](ROBUSTNESS.md#version-compatibility). |
| **7 — Specialist workflow roles** | Roadmap only | Add scout, planner, builder, tester, reviewer, and security roles as explicit steps. | [Capability ports](ROBUSTNESS.md#capability-ports), [Secret handling](ROBUSTNESS.md#secret-handling), and [Untrusted-input boundaries](ROBUSTNESS.md#untrusted-input-boundaries). |
| **8 — Parallel independent tasks** | Roadmap only | Execute proven-independent dependency levels concurrently in separate workspaces. | [Worktree lifecycle management](ROBUSTNESS.md#worktree-lifecycle-management), [Timeout, cancellation, and process cleanup](ROBUSTNESS.md#timeout-cancellation-and-process-cleanup), and [Multi-worker scheduling](ROBUSTNESS.md#multi-worker-scheduling). |
| **9 — Integration merge gate** | Roadmap only | Merge validated task results, use human conflict fallback, and validate the integrated result. | [Merge integrity](ROBUSTNESS.md#merge-integrity), [Stronger source-state checks](ROBUSTNESS.md#stronger-source-state-checks), and [Changed-path policy](ROBUSTNESS.md#changed-path-policy). |
| **10 — Specialized workflow registry** | Roadmap only | Define chore, bug, feature, hotfix, and security workflows with different steps and human gates. | [Contract tests](ROBUSTNESS.md#contract-tests), [Version compatibility](ROBUSTNESS.md#version-compatibility), and [Capability-based model aliases](ROBUSTNESS.md#capability-based-model-aliases). |
| **11 — Ticket and review-event intake** | Roadmap only | Start runs from tickets and route review or CI feedback to the correct active run. | [Authenticated event ingress](ROBUSTNESS.md#authenticated-event-ingress), [Event ordering and concurrency](ROBUSTNESS.md#event-ordering-and-concurrency), [Duplicate protection](ROBUSTNESS.md#duplicate-protection), and [Durable event execution](ROBUSTNESS.md#durable-event-execution). |
| **12 — Evaluation and regression workflow** | Roadmap only | Build versioned cases and compare workflow, agent, prompt, and tool variants before promotion. | [Versioned evaluation assets](ROBUSTNESS.md#versioned-evaluation-assets), [Promotion gates and canaries](ROBUSTNESS.md#promotion-gates-and-canaries), and [Distributed tracing](ROBUSTNESS.md#distributed-tracing). |
| **13 — Controlled expert-role packages** | Roadmap only | Version scoped instructions, examples, skills, retrieval policy, and candidate lessons. | [Lesson provenance](ROBUSTNESS.md#lesson-provenance), [Retrieval safety](ROBUSTNESS.md#retrieval-safety), and [Promotion gates and canaries](ROBUSTNESS.md#promotion-gates-and-canaries). |

A phase remains complete based on its functional acceptance behavior. Linked robustness work must not become a completion gate unless the owner explicitly promotes it.

---

## Phase 0 — Code-Defined Workflow Kernel

**Phase status:** implementation-ready / ready-for-agent

### Goal

Prove the smallest valuable vertical slice: a user requests a bounded change, a code-defined workflow invokes a coding worker in an isolated local workspace, deterministic validation runs outside the agent prompt, and the workflow stops at a human review gate with an inspectable result. The design must make it straightforward to add further workflow steps without rewriting the executor.

Build this phase as a local TypeScript and Node.js workflow kernel whose workflows are ordinary typed code.

The workflow contains three explicit step kinds:
...
This phase deliberately fails simply. Agent or validation failure ends the run as `failed` and leaves inspectable output. It does not add retries, persistence, recovery machinery, tracing infrastructure, containers, or other robustness features from `ROBUSTNESS.md`.

### User Stories

These user stories apply only to Phase 0. Future phases must define their own detailed stories before promotion to implementation-ready status.
...
18. As a local engineer, I want a passing workflow to stop at a Human Gate, so that I retain authority over integration.
...
45. As a product owner, I want robustness ideas kept outside this phase specification, so that optional technical work cannot silently become release scope.
...
- Do not route failed validation back to the coding worker in this phase.
...
- Model only `awaiting_review` and `failed` as terminal states in this phase.
...
- Preserve the ranked functional direction after this phase: typed planning, configurable validation, independent review, review feedback, role-specific context, decomposition, specialists, parallel execution, merge gating, specialized workflows, ticket intake, evaluation, and controlled expertise.
...
- a typed workflow runs Agent, Command, and Human Gate steps in declared order through the Workflow Executor interface;
- a valid request operates only in a disposable clone and leaves the source repository unchanged;
- dirty-source and expected-commit mismatches fail before workspace execution;
- Agent and validation failures stop later steps and return inspectable structured evidence;
- a successful run stops at `awaiting_review` with a commit or diff reference and manual integration guidance;
- an additional typed step can run without changing executor control flow; and
- the thin CLI can invoke the workflow and render its result.
...
## Promoting a Future Phase

Before a roadmap phase becomes implementation-ready:

1. the owner explicitly promotes it;
2. its brief is expanded into a phase specification with detailed user stories, implementation decisions, acceptance behavior, testing decisions, dependencies, and out-of-scope boundaries;
3. any `ROBUSTNESS.md` items required for that phase are explicitly promoted into its scope; and
4. implementation tickets are cut only from that approved phase specification.

The first candidate for promotion is Phase 1, Typed Change Plan. Later priorities remain configurable validation gates, independent review, review feedback, role-specific context packs, task decomposition, specialist roles, parallel execution, integration merging, specialized workflows, ticket intake, evaluation, and controlled expert-role packages.

## Further Notes

- `ROBUSTNESS.md` is the only companion document. It contains optional, mostly user-invisible technical ideas and is not part of Phase 0 implementation context.
...
- No Git remote or external issue tracker is configured for this repository. This file is therefore the local published specification; only Phase 0 currently carries `ready-for-agent` status.
