...
Each phase adds one visible workflow capability. The linked robustness ideas are the most relevant technical improvements to reconsider during that phase; they are recommendations, not automatic scope. An implementation agent may explain why one has become timely, but only the owner may promote it from `ROBUSTNESS.md` into implementation work.

| Phase | Functional increment | Most relevant optional robustness ideas |
| --- | --- | --- |
| **0 — Code-defined workflow kernel (current)** | Execute Agent, Command, and Human Gate steps in order and return a reviewable local result. | [Current minimum policy](ROBUSTNESS.md#current-minimum-policy), [Capability ports](ROBUSTNESS.md#capability-ports), [Contract tests](ROBUSTNESS.md#contract-tests), and [Local run evidence](ROBUSTNESS.md#local-run-evidence). |
| **1 — Typed Change Plan** | Add a planning step that produces objective, non-goals, expected files, risks, acceptance criteria, and validation commands. | [Untrusted-input boundaries](ROBUSTNESS.md#untrusted-input-boundaries) and [Local run evidence](ROBUSTNESS.md#local-run-evidence). |
| **2 — Configurable validation gates** | Add, remove, and reorder formatter, lint, type-check, test, and repository-command steps. | [Timeout, cancellation, and process cleanup](ROBUSTNESS.md#timeout-cancellation-and-process-cleanup), [Contract tests](ROBUSTNESS.md#contract-tests), and [Local run evidence](ROBUSTNESS.md#local-run-evidence). |
| **3 — Independent review** | Give a reviewer the plan, diff, and validation evidence before the Human Gate. | [Untrusted-input boundaries](ROBUSTNESS.md#untrusted-input-boundaries), [Changed-path policy](ROBUSTNESS.md#changed-path-policy), and [Local run evidence](ROBUSTNESS.md#local-run-evidence). |
| **4 — Review-to-build feedback** | Return review findings to the builder through an explicit workflow edge. | [Bounded correction attempts](ROBUSTNESS.md#bounded-correction-attempts), [Retry classification](ROBUSTNESS.md#retry-classification), and [Run-state persistence and resume](ROBUSTNESS.md#run-state-persistence-and-resume). |
| **5 — Role-specific context packs** | Give each step only the instructions, active requirements, and artifacts needed for its role. | [Untrusted-input boundaries](ROBUSTNESS.md#untrusted-input-boundaries), [Retrieval safety](ROBUSTNESS.md#retrieval-safety), and [Lesson provenance](ROBUSTNESS.md#lesson-provenance). |
| **6 — Task graph and bounded decomposition** | Split a plan into scoped tasks with dependencies, owners, acceptance criteria, allowed files, and validation plans. | [Timeout, cancellation, and process cleanup](ROBUSTNESS.md#timeout-cancellation-and-process-cleanup), [Changed-path policy](ROBUSTNESS.md#changed-path-policy), and [Version compatibility](ROBUSTNESS.md#version-compatibility). |
| **7 — Specialist workflow roles** | Add scout, planner, builder, tester, reviewer, and security roles as explicit steps. | [Capability ports](ROBUSTNESS.md#capability-ports), [Secret handling](ROBUSTNESS.md#secret-handling), and [Untrusted-input boundaries](ROBUSTNESS.md#untrusted-input-boundaries). |
| **8 — Parallel independent tasks** | Execute proven-independent dependency levels concurrently in separate workspaces. | [Worktree lifecycle management](ROBUSTNESS.md#worktree-lifecycle-management), [Timeout, cancellation, and process cleanup](ROBUSTNESS.md#timeout-cancellation-and-process-cleanup), and [Multi-worker scheduling](ROBUSTNESS.md#multi-worker-scheduling). |
| **9 — Integration merge gate** | Merge validated task results, use human conflict fallback, and validate the integrated result. | [Merge integrity](ROBUSTNESS.md#merge-integrity), [Stronger source-state checks](ROBUSTNESS.md#stronger-source-state-checks), and [Changed-path policy](ROBUSTNESS.md#changed-path-policy). |
| **10 — Specialized workflow registry** | Define chore, bug, feature, hotfix, and security workflows with different steps and human gates. | [Contract tests](ROBUSTNESS.md#contract-tests), [Version compatibility](ROBUSTNESS.md#version-compatibility), and [Capability-based model aliases](ROBUSTNESS.md#capability-based-model-aliases). |
| **11 — Ticket and review-event intake** | Start runs from tickets and route review or CI feedback to the correct active run. | [Authenticated event ingress](ROBUSTNESS.md#authenticated-event-ingress), [Event ordering and concurrency](ROBUSTNESS.md#event-ordering-and-concurrency), [Duplicate protection](ROBUSTNESS.md#duplicate-protection), and [Durable event execution](ROBUSTNESS.md#durable-event-execution). |
| **12 — Evaluation and regression workflow** | Build versioned cases and compare workflow, agent, prompt, and tool variants before promotion. | [Versioned evaluation assets](ROBUSTNESS.md#versioned-evaluation-assets), [Promotion gates and canaries](ROBUSTNESS.md#promotion-gates-and-canaries), and [Distributed tracing](ROBUSTNESS.md#distributed-tracing). |
| **13 — Controlled expert-role packages** | Version scoped instructions, examples, skills, retrieval policy, and candidate lessons. | [Lesson provenance](ROBUSTNESS.md#lesson-provenance), [Retrieval safety](ROBUSTNESS.md#retrieval-safety), and [Promotion gates and canaries](ROBUSTNESS.md#promotion-gates-and-canaries). |
