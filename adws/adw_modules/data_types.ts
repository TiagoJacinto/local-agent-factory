...
export function envelope(type: OutputType, value: any): EnvelopeBase {
	if (
		...
		throw new Error(
			"response is not a valid envelope: status must be success or fail",
		);
	...
	if (
		...
		(!Array.isArray(value.artifacts) ||
			value.artifacts.some((x: any) => typeof x !== "string"))
	)
		throw new Error("artifacts must be an array of strings");
	if (
		value.notes_for_next_agent !== undefined &&
		typeof value.notes_for_next_agent !== "string"
	)
	...
	if (type === "ReviewOutput") {
		...
		for (const f of value.findings || [])
			if (typeof f?.requirement !== "string" || typeof f?.met !== "boolean")
				throw new Error(
					"ReviewOutput.findings entries require requirement and met",
				);
	}
	if (type === "BuildOutput") {
		array("changed_files");
	...
	if (type === "ScoutOutput") {
		...
		for (const f of value.findings || [])
			if (typeof f?.file !== "string")
				throw new Error("ScoutOutput.findings entries require file");
	}
	if (
		type === "PlanOutput" &&
		value.commit_message !== undefined &&
	...
	if (type === "DocumentOutput") {
		...
		if (
			value.document_path !== undefined &&
			typeof value.document_path !== "string"
		)
			throw new Error("DocumentOutput.document_path must be a string");
	}
	if (type === "VerifyOutput") {
		if (typeof value.passed !== "boolean")
			throw new Error("VerifyOutput.passed must be boolean");
		array("failures");
	}
