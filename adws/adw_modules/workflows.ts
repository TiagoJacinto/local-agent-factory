...
import { PhaseParams } from "./data_types";
...
export async function scout(x: any) {
	...
	await run.phase(
		...
		(ph) =>
			ph.call(
				new AgentCall("ScoutOutput", x.prompt, undefined, [
...
export async function build(x: any) {
	...
	await run.phase(
		...
		(ph) =>
			ph.call(
				new AgentCall("BuildOutput", x.prompt, undefined, [
			...
			ph.log({
				sha: git.commitAll(
					build.commit_message ||
						`sssf(${run.adwId}): ${build.summary || "build"}`,
					run.repoRoot,
				),
			}),
	);
	...
	if (r.test.passed)
		await run.phase(
			...
			(ph) =>
				ph.log({
					sha: git.commitAll(
						r.previous.commit_message ||
							`sssf(${run.adwId}): ${r.previous.summary || "build"}`,
						run.repoRoot,
					),
				}),
		);
...
export async function buildReview(x: any) {
	...
	for (let i = 1; i <= 3; i++) {
		await run.phase(
			{
				...
				owner: "reviewer",
				description:
					"Rule on every requirement in the spec, against the code on disk",
			},
			async (ph) =>
		...
		await run.phase(
			...
			async (ph) =>
				(prev = await ph.call(
					new AgentCall("BuildOutput", x.prompt, review, [
						gates.diffMatchesClaims,
					]),
				)),
		);
...
export async function qualityRun(x: any) {
	...
	await run.phase(
		...
		async (ph) => {
			...
			if (!r.passed)
				throw new Error(`quality failed: ${r.failures.join("; ")}`);
		},
	);
	return run.finish();
}
export async function document(x: any) {
	...
	await run.phase(
		...
		(ph) => {
			...
			if (!c.files.length && !c.untracked.length)
				throw new Error("nothing changed to document");
		},
	);
	await run.phase(
		{
			...
			retries: 1,
			description:
				"Turn the captured diff into a write-up an engineer can read",
		},
		(ph) =>
			ph.call(
				new AgentCall(
...
export async function planBuildTest(x: any) {
	...
	await run.phase(
		...
		async (ph) =>
			(b = await ph.call(
				new AgentCall("BuildOutput", x.prompt, p, [gates.diffMatchesClaims]),
			)),
	);
	const r = await testLoop(run, x.prompt, b);
	if (r.test.passed)
...
export async function planBuildTestQuality(x: any) {
			...
			async (ph) =>
				(b = await ph.call(
					new AgentCall(
						"BuildOutput",
						x.prompt,
						quality.asEnvelope(q, "verification"),
						[gates.diffMatchesClaims],
					),
				)),
		);
	...
	return q?.passed
		...
		: run.finish(
				false,
				"verify/test never came back clean after 3 fix attempt(s)",
			);
}
	...
	await run.phase(
		...
		(ph) =>
			ph.log({
				sha: git.commitAll(
					plan.commit_message ||
						`sssf(${run.adwId}): ${plan.summary || "plan"}`,
					run.repoRoot,
				),
			}),
	);
		...
		await run.phase(
			{
				...
				owner: "git",
				description:
					"Land the code only after a green suite and approved review",
			},
			(ph) =>
				ph.log({
					sha: git.commitAll(
		...
		await run.phase(
			...
			(ph) =>
				ph.log({
					sha: git.commitAll(
						doc.commit_message ||
							`sssf(${run.adwId}): ${doc.summary || "docs"}`,
						run.repoRoot,
					),
				}),
		);
