...
export function resolveBase(ref = "main", cwd = process.cwd()): BaseRef {
	if (!git.isRepo(cwd))
		throw new Error("not a git repository — change capture needs one");
	if (!git.refExists(ref, cwd))
		throw new Error(`base ref ${ref} does not exist`);
	...
	if (git.shortSha(commit, cwd) !== git.shortSha("HEAD", cwd))
		...
		reason = `HEAD is on ${label} — diffing the uncommitted working tree`;
...
export function capture(run: any, p: ChangeCapture = {}): ChangeSet {
	...
	const untracked =
		p.includeUntracked === false ? [] : git.untrackedFiles(run.repoRoot);
