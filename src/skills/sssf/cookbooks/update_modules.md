...
Do not add provider calls, subprocesses, or persistence to `run.ts`. Keep those concerns in the Factory modules; `run.ts` only invokes the selected workflow ID. Use typed ports so tests can inject deterministic adapters. Every phase has a meaningful description and every artifact handoff names its producer and consumer. Run typecheck and focused tests after changes.
