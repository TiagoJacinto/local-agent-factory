...
| `agents.ts`     | `load_config`, `validate`, resolving an entry → coding-agent interface + model + thinking + built-in tools                                                                                                                         |
...
```typescript
import { GateReport } from "../templates/adws/adw_modules/data_types"

function testsDeclaredPassed(envelope: any, run: any): GateReport {
