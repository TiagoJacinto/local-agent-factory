...
The full `sssf.config.yaml` spec: every field, how defaults merge, and how model / thinking / built-in tools map onto the coding agent.
...
| `coding_agent`, `model`, `thinking`, `color` | no       | Override the corresponding `defaults` key.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
`tools` maps to `pi --tools`. The factory allows only Pi's built-in tools:

| Tool    | Purpose                   | Pi's own default |
| ------- | ------------------------- | ---------------- |
| `todo`  | track implementation work | on               |
...
`grep`, `find`, and `ls` are off in bare Pi, so an agent that does not name them will shell out through `bash` to do the same work. The starter roster sets `defaults.tools` to the built-in read/write/search surface and lets each agent narrow from there.
...
The runtime has no extension or custom-tool loading path. If a capability is needed, implement it as deterministic workflow code or use one of the built-in Pi tools.
