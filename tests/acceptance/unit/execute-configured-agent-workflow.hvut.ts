...
const feature =
	loadFeatureFromText(`Feature: Execute a configured agent workflow
...
function expectInvocations(
	...
	expect(
		run.invocations.map(({ order, name, primitiveType, status }) => ({
		...
		rows.map(({ invocationOrder, name, primitiveType, status }) => ({
			invocationOrder,
			name,
			primitiveType,
			status,
...
describeFeature(feature, ({ Rule }) => {
	Rule(
		"Run known validation as code and return inspectable evidence",
		({ RuleScenario }) => {
			RuleScenario(
				"Stop before review when validation fails",
				({ Given, When, Then, And }) => {
					...
					And(
						sourceStep,
						(_ctx: unknown, path: string, revision: string, state: string) => {
					...
					Then(
						...
						(_ctx: unknown, status: string, failure: string) =>
	...
	Rule(
		"Correct a workflow without losing the active agent session",
		({ RuleScenario }) => {
			RuleScenario(
				"Send a review finding back to the builder",
				({ Given, When, Then, And }) => {
					let executor: WorkflowExecutor;
					let source: { path: string; revision: string };
					let run: Awaited<ReturnType<WorkflowExecutor["executeWorkflow"]>>;
					Given(workflowStep, (_ctx: unknown, workflowList: string) => {
						...
						executor = new WorkflowExecutor(
							createStarterWorkflowDefinitions(),
							{
								...
								ai: ({ name }) => ({
									value:
										name === "Review change" && firstReview
					...
					And(
						...
						(_ctx: unknown, sameAgentContext: string) =>
							expect(run.session.sameAgentContext).toBe(
					...
					And(
						...
						(
							...
							expect(run.context.envelopes.get("review")).toMatchObject({
								producer,
								consumer,
								status,
							}),
	...
	Rule(
		"Carry typed workflow envelopes and artifacts between roles",
		({ RuleScenario }) => {
			RuleScenario(
				"Pass a plan envelope to the builder",
				({ Given, When, Then, And }) => {
					let executor: WorkflowExecutor;
					let source: { path: string; revision: string };
					let run: Awaited<ReturnType<WorkflowExecutor["executeWorkflow"]>>;
	...
	Rule("Enforce role boundaries", ({ RuleScenario }) => {
		RuleScenario(
			"Reject an unauthorized repository change",
			({ Given, And, When, Then }) => {
				let executor: WorkflowExecutor;
				let source: { path: string; revision: string };
				let run: Awaited<ReturnType<WorkflowExecutor["executeWorkflow"]>>;
						...
						roles: [
							{
								...
								tools: ["read"],
								allowedWrites: [],
								protectedPaths: ["workflow-engine"],
							},
						],
						ai: ({ workspacePath }) => {
							writeFileSync(
								join(workspacePath ?? "", "workflow-engine"),
								"changed\\n",
							);
							return {
								value: {
				...
				And(
					"AgentRole{name: {string}, allowedWrites: {any}, protectedPaths: {any}}",
					() => undefined,
				);
				And(sourceStep, () => undefined);
				When(
					'I executeWorkflow(workflowId: "review", sourceRepository: "/work/project", expectedSourceRevision: "abc123")',
					async () => {
						run = await executor.executeWorkflow("review", {
							sourceRepository: source.path,
							expectedSourceRevision: source.revision,
						});
					},
				...
				Then(
					"I view WorkflowRun{status: {word}, failure: {word}} in Workflow Execution: Unauthorized changes fail the role phase",
					(_ctx: unknown, status: string, failure: string) =>
						expect(run).toMatchObject({ status, failure }),
				);
				And(
					"I view WorkflowEnvelope{status: {word}, summary: {string}} in Workflow Execution: Permission failure is reported",
					(_ctx: unknown, _status: string, summary: string) => {
	...
	Rule(
		"Return a reviewable result without automatic integration",
		({ RuleScenario }) => {
			RuleScenario(
				"Finish with a commit or diff and manual integration guidance",
				({ Given, And, When, Then }) => {
					let executor: WorkflowExecutor;
					let source: { path: string; revision: string };
					let run: Awaited<ReturnType<WorkflowExecutor["executeWorkflow"]>>;
					Given(workflowStep, () => {
						...
						executor = new WorkflowExecutor(
							createStarterWorkflowDefinitions(),
							{
								roles: ["builder", "reviewer", "documenter"].map((name) => ({
									name,
									model: "test",
									instructions: name,
									tools: ["read"],
					...
					And(sourceStep, () => undefined);
					When(
						'I executeWorkflow(workflowId: "simple-sdlc", sourceRepository: "/work/project", expectedSourceRevision: "abc123")',
						async () => {
							run = await executor.executeWorkflow("simple-sdlc", {
								sourceRepository: source.path,
								expectedSourceRevision: source.revision,
							});
						},
					...
					Then(
						"I view WorkflowRun{status: {word}, sourceRepositoryUnchanged: true, integration: Manual} in Workflow Execution: Human review is required",
						(_ctx: unknown, status: string) =>
					...
					And(
						"I view AgentRole{name: {string}, output: Present} in Workflow Execution: The completed change has a documentation phase",
						(_ctx: unknown, name: string) =>
							expect(
								run.invocations.some(
									(invocation) =>
										invocation.name === "Document change" &&
					...
					And(
						"I view Artifact{kind: ReviewableChange, reference: Present} in Workflow Execution: The change can be reviewed",
						() =>
							expect(
								run.context.artifacts.get("reviewable-change"),
							).toMatchObject({ kind: "ReviewableChange" }),
					);
					And(
						"I !view Artifact{kind: AutomaticIntegration} in Workflow Run: The factory does not integrate automatically",
						() =>
							expect(
								Array.from(run.context.artifacts.values()).some(
									(artifact) =>
										artifact.kind === undefined &&
										artifact.id === "automatic-integration",
								),
							).toBe(false),
					);
				},
