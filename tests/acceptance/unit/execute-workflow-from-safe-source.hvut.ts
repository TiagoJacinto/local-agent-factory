...
function createRepository(): { path: string; revision: string; head: string } {
	...
	const revision = execFileSync("git", ["-C", path, "rev-parse", "HEAD"], { encoding: "utf8" }).trim();
...
describe("Execute a workflow from a safe source repository", () => {
	test("executes worker activity in a disposable workspace", async () => {
		...
		expect(run).toMatchObject({ status: "Succeeded", sourceRevision: source.revision, workspaceIsolation: "IndependentClone", sourceIntegrity: "Verified" });
		expect(run.runIdentifier).toMatch(/^local-run-/);
		expect(run.workspacePath).toBe(workerWorkspace);
		expect(run.invocations[0]).toMatchObject({ invocationId: "update", status: "Succeeded", workspacePath: workerWorkspace });
		expect(readFileSync(join(run.workspacePath!, "README.md"), "utf8")).toBe("after\n");
		expect(execFileSync("git", ["-C", source.path, "rev-parse", "HEAD"], { encoding: "utf8" }).trim()).toBe(source.head);
		expect(execFileSync("git", ["-C", source.path, "status", "--porcelain"], { encoding: "utf8" })).toBe("");
	});
	...
	test("rejects a dirty source before workspace or worker execution", async () => {
		...
		const executor = new WorkflowExecutor([workflow()], { harness: () => { workerCalled = true; return undefined; } });
		const run = await executor.executeWorkflow("update-readme", { sourceRepository: source.path, expectedSourceRevision: source.revision });

		expect(run).toMatchObject({ status: "Failed", sourceRevision: source.revision, sourceIntegrity: "Verified", failure: "DirtySource" });
	...
	test("rejects an unexpected source revision before workspace or worker execution", async () => {
		...
		let workerCalled = false;
		const executor = new WorkflowExecutor([workflow()], { harness: () => { workerCalled = true; return undefined; } });
		const run = await executor.executeWorkflow("update-readme", { sourceRepository: source.path, expectedSourceRevision: "unexpected-revision" });

		expect(run).toMatchObject({ status: "Failed", sourceRevision: source.revision, sourceIntegrity: "Verified", failure: "UnexpectedSourceRevision" });
		expect(run.workspacePath).toBeUndefined();
		expect(run.invocations).toEqual([]);
		expect(workerCalled).toBe(false);
	});
	...
	test("retains the disposable workspace when worker activity fails", async () => {
		...
		const run = await executor.executeWorkflow("update-readme", { sourceRepository: source.path, expectedSourceRevision: source.revision, workspaceRoot });

		expect(run).toMatchObject({ status: "Failed", sourceRevision: source.revision, workspaceDisposition: "Retained" });
		expect(run.invocations[0]).toMatchObject({ status: "Failed" });
		expect(readFileSync(join(run.workspacePath!, "failure.txt"), "utf8")).toBe("inspect me\n");
		expect(execFileSync("git", ["-C", source.path, "rev-parse", "HEAD"], { encoding: "utf8" }).trim()).toBe(source.head);
		expect(execFileSync("git", ["-C", source.path, "status", "--porcelain"], { encoding: "utf8" })).toBe("");
	});
});
