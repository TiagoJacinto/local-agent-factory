      ...
      ({ Given, When, Then, And, But }) => {
        ...
        But(
          ...
          (_ctx: unknown, functionName: string) =>
            expect(run.invocations.some((invocation) => invocation.name === functionName)).toBe(
              false,
            ),
        );
      },
      ...
      ({ Given, When, Then, And, But }) => {
        ...
        Given(workflowStep, (_ctx: unknown, id: string, name: string, rows: Row[]) => {
          ...
          const verifyThatREADMEFollowsRepoRules = async (
            invoke: (id: string, name: string, input: string) => Promise<unknown>,
          ) => invoke(row.invocationId, row.invocationName, "Verify repository rules");
          const workflow: WorkflowDefinition = {
            id,
            name,
