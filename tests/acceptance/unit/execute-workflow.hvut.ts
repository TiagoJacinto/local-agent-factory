...
function expectRun(
	...
	const expected = rows.map((row) => ({
		...
		...(row.consumedArtifact && row.consumedArtifact !== "none"
			? { consumedArtifact: row.consumedArtifact }
			: {}),
		...(row.producedArtifact && row.producedArtifact !== "none"
			? { producedArtifact: row.producedArtifact }
			: {}),
	}));
	expect(
		invocationSummary(run).map((invocation, index) =>
			Object.fromEntries(
				Object.keys(expected[index]).map((key) => [
					key,
					invocation[key as keyof typeof invocation],
				]),
			),
		),
	).toEqual(expected);
...
describeFeature(feature, ({ Rule }) => {
	Rule(
		"Record primitive invocations in controller order",
		({ RuleScenario }) => {
			RuleScenario(
				"Execute AI, Harness, and Gate primitives",
				({ Given, When, Then }) => {
					let executor: WorkflowExecutor;
					let workflowRun: Awaited<
						ReturnType<WorkflowExecutor["executeWorkflow"]>
					>;
					Given(
						workflowStep,
						(_ctx: unknown, id: string, name: string, rows: Row[]) => {
							...
							const workflow: WorkflowDefinition = {
								...
								controller: async ({ ai, harness, gate }) => {
									for (const row of rows) {
										const primitive = { AI: ai, Harness: harness, Gate: gate }[
											row.function as "AI" | "Harness" | "Gate"
										];
					...
					Then(
						...
						(_ctx: unknown, id: string, status: string, rows: Row[]) =>
							expectRun(workflowRun, id, status, rows),
					);
				},
			);
		},
	...
	Rule("Carry artifacts between primitive invocations", ({ RuleScenario }) => {
		RuleScenario(
			"Harness receives an artifact produced by AI",
			({ Given, When, Then, And }) => {
				let executor: WorkflowExecutor;
						...
						const workflow: WorkflowDefinition = {
							...
							controller: async ({ ai, harness }) => {
								for (const row of rows) {
									...
									await primitive(row.invocationId, row.name, row.input, {
										inputArtifact:
											row.inputArtifact === "none"
												? undefined
												: row.inputArtifact,
										outputArtifact:
											row.outputArtifact === "none"
												? undefined
												: row.outputArtifact,
									});
								}
	...
	Rule("Compose ordinary code with workflow primitives", ({ RuleScenario }) => {
		RuleScenario(
			...
			({ Given, When, Then, And, But }) => {
					...
					(_ctx: unknown, id: string, name: string, rows: Row[]) => {
						...
						const workflow: WorkflowDefinition = {
							...
							controller: async ({ ai }) => {
								...
								await ai(primitive.invocationId, primitive.name, prompt, {
									outputArtifact: primitive.output,
								});
							},
						};
						executor = new WorkflowExecutor([workflow]);
				...
				And(
					"I view AIInvocationResult{invocationId: {string}, prompt: {string}} in Workflow Execution: Pure computation output reaches AI",
					(_ctx: unknown, id: string, prompt: string) =>
					...
					(_ctx: unknown, functionName: string) =>
						expect(
							run.invocations.some(
								(invocation) => invocation.name === functionName,
							),
						).toBe(false),
				);
			},
		...
		RuleScenario(
			...
			({ Given, When, Then, And, But }) => {
				...
				And(
					"I view HarnessInvocationResult{invocationId: {string}, status: {word}} in Workflow Execution: Composite function delegates to Harness",
					(_ctx: unknown, id: string, status: string) =>
						expect(run.invocations[0]).toMatchObject({
							invocationId: id,
							status,
						}),
