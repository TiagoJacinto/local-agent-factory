	...
	return [
		{
			...
			controller: async ({ ai, harness }) => {
				...
				await harness("builder", "Build request", "build", {
					inputArtifact: "plan",
...
describeFeature(feature, ({ Rule }) => {
	Rule("Resume the active workflow context", ({ RuleScenario }) => {
		RuleScenario(
			"Continue a workflow with its existing agent context",
			({ Given, When, Then, And }) => {
				let setup: TestSetup;
				...
				Given(
					...
					(_ctx: unknown, runIdentifier: string, status: string) => {
						...
						sessionStorePath = mkdtempSync(
							join(tmpdir(), "workflow-sessions-"),
						);
						setup = {
							executor: new WorkflowExecutor(
								createSessionWorkflows(),
								{
				...
				And(
					...
					(_ctx: unknown, runIdentifier: string, sameAgentContext: string) =>
						expect(setup.run.session).toMatchObject({
							runIdentifier,
							sameAgentContext: sameAgentContext === "true",
						}),
				);
				And(
					...
					() =>
						expect([...setup.run.context.artifacts.values()]).toEqual(
	...
	Rule(
		"Keep the session record across chained workflows",
		({ RuleScenario }) => {
			RuleScenario(
				"Start the next workflow in an existing session",
				({ Given, When, Then, And }) => {
					...
					When(
						...
						async () => {
							await executor.executeWorkflow("correctable", {
								runIdentifier: "run-002",
							});
							run = await executor.executeWorkflow("document", {
								runIdentifier: "run-002",
							});
						},
					);
